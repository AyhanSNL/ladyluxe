...
$fontlar->execute();
$sayfaSorgu = $db->prepare("select * from video_ayar where id='1' ");
$sayfaSorgu->execute();
$detay = $sayfaSorgu->fetch(PDO::FETCH_ASSOC);
?>
<title><?=$diller['adminpanel-menu-text-175']?> - <?=$panelayar['baslik']?></title>
                    ...
                    <div class="row align-items-center d-flex justify-content-between" >
                        <div class="col-md-8" >
                            <div class="page-title-nav">
                                ...
                                <a href="pages.php?page=theme_videos"><i class="fa fa-angle-right"></i> <?=$diller['adminpanel-menu-text-175']?></a>
                            </div>
                        </div>
                        <div class="col-md-auto mr-3" >
                            <?php if($yetki['icerik_yonetim'] == '1' && $yetki['galeri'] == '1' ) {?>
                                <div class="mt-2 d-md-none d-sm-block"></div>
                                <a href="pages.php?page=video_gallery"  class="btn btn-primary" style="font-size: 13px; font-weight: 400;"> <?=$diller['adminpanel-form-text-618']?></a>
                            <?php } ?>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card mb-2 " >
                            <div class="card-body">
                                <div class="w-100 d-flex align-items-center justify-content-between flex-wrap  mb-2">
                                    <h4> <?=$diller['adminpanel-menu-text-175']?></h4>
                                </div>
                                <div class="collapse in show" id="genelAcc" data-parent="#accordion">
                                    <div class="w-100 border-top pt-3">
                                        <form action="post.php?process=theme_videos_post&status=main_update" method="post">
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-6">
                                                    <label  for="font_secim" class="w-100"><?=$diller['adminpanel-form-text-77']?></label>
                                                    <select name="font_secim" class="form-control" id="font_secim" >
                                                        <?php foreach ($fontlar as $font) {?>
                                                            <option value="<?=$font['font_adi']?>" <?php if($font['font_adi'] == $detay['font_secim'] ) { ?>selected<?php }?>><?=$font['font_adi']?></option>
                                                        <?php }?>
                                                    </select>
                                                </div>
                                                <div class="form-group col-md-6 mb-4">
                                                    <label  for="izlenme" class="w-100" ><?=$diller['adminpanel-form-text-619']?></label>
                                                    <div class="custom-control custom-switch custom-switch-lg">
                                                        <input type="hidden" name="izlenme" value="0"">
                                                        <input type="checkbox" class="custom-control-input" id="izlenme" name="izlenme" value="1"  <?php if($detay['izlenme'] == '1'  ) { ?>checked<?php }?> ">
                                                        <label class="custom-control-label" for="izlenme"></label>
                                                    </div>
                                                </div>
                                                <div class="form-group col-md-6 mb-4">
                                                    <label  for="nav" class="w-100 d-flex justify-content-start align-items-center flex-wrap">
                                                    ...
                                                    <div class="custom-control custom-switch custom-switch-lg">
                                                        ...
                                                        <input type="checkbox" class="custom-control-input" id="nav" name="nav" value="1" <?php if($detay['nav'] == '1'  ) { ?>checked<?php }?> >
                                                        <label class="custom-control-label" for="nav"></label>
                                                    </div>
                                                </div>
                                                <div class="form-group col-md-6 mb-4">
                                                    <label  for="spot_akordion" class="w-100" ><?=$diller['adminpanel-form-text-621']?></label>
                                                    <div class="custom-control custom-switch custom-switch-lg">
                                                        <input type="hidden" name="spot_akordion" value="0"">
                                                        <input type="checkbox" class="custom-control-input" id="spot_akordion" name="spot_akordion" value="1"  <?php if($detay['spot_akordion'] == '1'  ) { ?>checked<?php }?> ">
                                                        <label class="custom-control-label" for="spot_akordion"></label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="in-header-page-main mt-4" >
                                                <div class="in-header-page-text">
                                                    ...
                                                    <?=$diller['adminpanel-text-311']?> <i class="ti-help-alt text-primary " data-toggle="tooltip" data-placement="top" title="<?=$diller['adminpanel-form-text-620']?>"></i>
                                                </div>
                                            </div>
                                            <div class="row mt-3">
