...
if($yetki['demo'] != '1' ) {
    if (isset($_GET['status'])) {
        if($_GET['status'] == 'main_update' || $_GET['status'] == 'bg_update' || $_GET['status'] == 'bg_delete' || $_GET['status'] == 'logo' || $_GET['status'] == 'logo_delete' ){
            ...
            if($_GET['status'] == 'main_update'  ) {
                if($_POST && isset($_POST['update'])  ) {
                    $guncelle = $db->prepare("UPDATE tanitim_video SET
                                     padding=:padding,
                                     margin=:margin,
                                     ...
                                     button_hover_renk=:button_hover_renk,
                                     button_ani_renk=:button_ani_renk,
                                     button_play_renk=:button_play_renk
                                     WHERE id='1'      
                                    ");
                    $sonuc = $guncelle->execute(array(
                        ...
                        'button_renk' => colorFormat($_POST['button_renk']),
                        'button_hover_renk' => colorFormat($_POST['button_hover_renk']),
                        'button_ani_renk' => colorFormat($_POST['button_ani_renk']),
                        'button_play_renk' => colorFormat($_POST['button_play_renk'])
                    ));
                    if($sonuc){
                        header('Location:'.$ayar['panel_url'].'pages.php?page=theme_intro');
            ...
            if($_GET['status'] == 'bg_delete'  ) {
                $sorgu = $db->prepare("select * from tanitim_video where id='1' ");
                $sorgu->execute();
            ...
            if( $_GET['status'] == 'logo_delete' ) {
                ...
                $sorgu->execute();
                $row = $sorgu->fetch(PDO::FETCH_ASSOC);
                unlink("../images/uploads/$row[logo]");
                $guncelle = $db->prepare("UPDATE tanitim_video SET
                            logo=:logo
                     WHERE id='1'      
                    ");
