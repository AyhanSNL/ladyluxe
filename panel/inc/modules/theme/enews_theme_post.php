    ...
    if (isset($_GET['status'])) {
        if($_GET['status'] == 'main_update' || $_GET['status'] == 'bg_update' || $_GET['status'] == 'bg_delete' ){
                ...
                /* Genel Update */
                            if($_GET['status'] == 'main_update'  ) {
                                if($_POST && isset($_POST['update'])  ) {
                                    $guncelle = $db->prepare("UPDATE ebulten_ayar SET
                                     baslik_renk=:baslik_renk,      
                                     padding=:padding,
                                     spot_renk=:spot_renk,
                                     margin=:margin,
                                     baslik_font=:baslik_font,
                                     ebulten_border=:ebulten_border,
                                     input_bg=:input_bg,
                                     input_border=:input_border,
                                     input_yazi_renk=:input_yazi_renk,
                                     input_button_yazi=:input_button_yazi
                                     WHERE id='1'      
                                    ");
                                    $sonuc = $guncelle->execute(array(
                                        ...
                                        'ebulten_border' => colorFormat($_POST['ebulten_border']),
                                        'input_bg' => colorFormat($_POST['input_bg']),
                                        'input_border' => colorFormat($_POST['input_border']),
                                        'input_yazi_renk' => colorFormat($_POST['input_yazi_renk']),
                                        'input_button_yazi' => colorFormat($_POST['input_button_yazi'])
                                    ));
                                    if($sonuc){
                                        header('Location:'.$ayar['panel_url'].'pages.php?page=theme_enews');
            ...
            if($_GET['status'] == 'bg_delete'  ) {
                $sorgu = $db->prepare("select * from ebulten_ayar where id='1' ");
                $sorgu->execute();
