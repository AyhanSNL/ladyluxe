        ...
        if($_GET['status'] == 'main_update' || $_GET['status'] == 'bg_update' || $_GET['status'] == 'bg_delete' ){
            ...
            if($_GET['status'] == 'main_update'  ) {
                if($_POST && isset($_POST['update'])  ) {
                    $guncelle = $db->prepare("UPDATE sayac_ayar SET
                                     padding=:padding,
                                     margin=:margin,
                                     yazi_font=:yazi_font,
                                     yazi_space=:yazi_space,
                                     border=:border,
                                     sayacayar_border=:sayacayar_border,
                                     yazi_renk=:yazi_renk
                                     WHERE id='1'      
                                    ");
                    $sonuc = $guncelle->execute(array(
                        ...
                        'margin' => $_POST['margin'],
                        'yazi_font' => $_POST['yazi_font'],
                        'yazi_space' => $_POST['yazi_space'],
                        'border' => $_POST['border'],
                        'sayacayar_border' => colorFormat($_POST['sayacayar_border']),
                        'yazi_renk' => colorFormat($_POST['yazi_renk'])
                    ));
                    if($sonuc){
                        header('Location:'.$ayar['panel_url'].'pages.php?page=theme_client_counter');
            ...
            if($_GET['status'] == 'bg_delete'  ) {
                $sorgu = $db->prepare("select * from sayac_ayar where id='1' ");
                $sorgu->execute();
