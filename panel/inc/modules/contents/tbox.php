    ...
    if ($yetki['demo'] != '1') {
        ...
        foreach ($position as $idler) {
            ...
            try {
                ...
                $query = $db->query("UPDATE ticaret_bilgi SET sira = '$count' WHERE id = '$idler2'");
...
?>
<title><?=$diller['adminpanel-menu-text-65']?> - <?=$panelayar['baslik']?></title>
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
                    ...
                    <div class="row align-items-center" >
                        <div class="col-md-8" >
                            <div class="page-title-nav">
                                ...
                                <a href="pages.php?page=commerce_boxes"><i class="fa fa-angle-right"></i> <?=$diller['adminpanel-menu-text-65']?></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php if($yetki['icerik_yonetim'] == '1' && $yetki['icerik_diger'] == '1' ) {
            ...
            $Say = $db->query("select * from ticaret_bilgi where dil='$_SESSION[dil]'");
            $ToplamVeri = $Say->rowCount();
            $Limit = 30;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from ticaret_bilgi where dil='$_SESSION[dil]'  order by sira ASC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <h4> <?=$diller['adminpanel-menu-text-65']?></h4>
                                <?=$diller['adminpanel-form-text-1124']?> <?=$ToplamVeri?>
                            </div>
                            <div class="new-buttonu-main">
                                <a  class="btn btn-info text-white  " href="pages.php?page=theme_tbox"><?=$diller['adminpanel-form-text-838']?></a>
                                <a  class="btn btn-success text-white "   data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-1179']?></a>
                            </div>
                        </div>
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse" id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3 mt-3">
                                    <form action="post.php?process=tbox_post&status=add" method="post" >
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-12 text-center bg-white text-dark  border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-1179']?></h5>
                                            </div>
                                        </div>
                                        <div class="row mt-3 ">
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-8">
                                                <label for="baslik">* <?=$diller['adminpanel-form-text-1180']?></label>
                                                <input type="text" autocomplete="off"  name="baslik" id="baslik" required class="form-control">
                                            </div>
                                            <div class="form-group col-md-8">
                                                <label for="spot">* <?=$diller['adminpanel-form-text-1181']?></label>
                                                <input type="text" autocomplete="off"  name="spot" id="spot" required class="form-control">
                                            </div>
                                            <div class="form-group col-md-8">
                                                <label for="icon" class="w-100 d-flex align-items-center justify-content-start flex-wrap">
                                                    <?=$diller['adminpanel-form-text-675']?>
                                                    <a href="" data-toggle="modal" data-target=".ikon_adresleri"  class="ml-2">
                                                        <i class="fa fa-external-link"></i>
                                                    </a>
                                                </label>
                                                <input type="text" autocomplete="off"  name="icon" id="icon"  class="form-control">
                                            </div>
                                        </div>
                                    ...
                                    <div class="<?php if($panelayar['panel_nav'] == '1'   ) { ?>col-md-6<?php }else{?>col-md-4<?php } ?>" id="<?php echo $row['id'] ?>" style="cursor: move">
                                        <div class="card border p-4 ">
                                            <div class="d-flex align-items-center justify-content-between ">
                                                <div class=" d-flex align-items-center justify-content-start " >
                                                    <i class="fa <?=$row['icon']?> mr-3 mt-2" style="font-size: 30px ;"></i>
                                                    <div>
                                                        <span style="font-size: 14px ; font-weight: 500;"><?=$row['baslik']?></span>
                                                        <br>
                                                        <?=$row['spot']?>
                                                    </div>
                                                </div>
                                                <div style="min-width: 90px; text-align: right;">
                                                    ...
                                                    <a href="" data-href="post.php?process=tbox_post&status=delete&no=<?=$row['id']?>"  data-toggle="modal" data-target="#confirm-delete"  class="btn btn-sm btn-danger "><i class="fa fa-trash" ></i></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light border-top  ">
                                ...
                                <nav aria-label="Page navigation example " >
                                    <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                        <?php } ?>
                                        <?php if($Sayfa > 1){  ?>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=commerce_boxes"><?=$diller['sayfalama-ilk']?></a></li>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=commerce_boxes&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                        <?php } ?>
                                        <?php
                                        for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                            if($i == $Sayfa){
                                                ...
                                                <li class="page-item active " aria-current="page">
                                                    <a class="page-link" href="pages.php?page=commerce_boxes&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                </li>
                                                <?php
                                            }else{
                                                ?>
                                                <li class="page-item "><a class="page-link" href="pages.php?page=commerce_boxes&p=<?=$i?>"><?=$i?></a></li>
                                                <?php
                                            }
                                        }
                                        }
                                        ...
                                        <?php if($islemListele->rowCount() <=0) { } else { ?>
                                            <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=commerce_boxes&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=commerce_boxes&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                            <?php }} ?>
                                        <?php if($Sayfa >= 1){?>
                                    </ul>
                                </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=tbox_edit',
                type: 'post',
                data: {postID: postID},
