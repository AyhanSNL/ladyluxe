    ...
    if ($yetki['demo'] != '1') {
        ...
        foreach ($position as $idler) {
            ...
            try {
                ...
                $query = $db->query("UPDATE galeri_kat SET sira = '$count' WHERE id = '$idler2'");
...
if ($yetki['demo'] != '1') {
    if ($_GET['status_update'] == !null) {
        ...
        $statusCek = $db->prepare("select * from galeri_kat where id=:id ");
        ...
        if ($statusCek->rowCount() > '0') {
         ...
         ");
            ...
            if ($sonuc) {
                header('Location:' . $ayar['panel_url'] . 'pages.php?page=photo_gallery' . $sayfa . ''.$searchPage.'');
            } else {
...
?>
<title><?=$diller['adminpanel-menu-text-59']?> - <?=$panelayar['baslik']?></title>
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
        ...
        <?php if($yetki['icerik_yonetim'] == '1' && $yetki['galeri'] == '1' ) {
            ...
            $Say = $db->query("select * from galeri_kat where dil='$_SESSION[dil]' $search");
            $ToplamVeri = $Say->rowCount();
            $Limit = 27;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from galeri_kat where dil='$_SESSION[dil]' $search order by sira ASC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top border-bottom pb-2">
                            <div class="new-buttonu-main-left">
                                <h4> <?=$diller['adminpanel-menu-text-59']?></h4>
                                <?=$diller['adminpanel-form-text-1124']?> <?=$ToplamVeri?>
                            </div>
                            <div class="new-buttonu-main">
                                <a  class="btn btn-info text-white " href="pages.php?page=theme_gallery"><?=$diller['adminpanel-form-text-838']?></a>
                                <a  class="btn btn-success text-white "   data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-1137']?></a>
                            </div>
                        </div>
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse " id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3">
                                    <form action="post.php?process=photo_gallery_post&status=album_add" method="post" enctype="multipart/form-data" >
                                        <div class="row ">
                                            <div class="form-group col-md-12 text-center bg-white text-dark  border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-1137']?></h5>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row mt-3">
                                            ...
                                            <div class="form-group col-md-4 mb-4">
                                                <label  for="anasayfa" class="w-100 d-flex align-items-center justify-content-start" ><?=$diller['adminpanel-form-text-1080']?> <i class="ti-help-alt text-primary float-right ml-2" data-toggle="tooltip" data-placement="top" title="<?=$diller['adminpanel-form-text-1140']?>"></i></label>
                                                <div class="custom-control custom-switch custom-switch-lg">
                                        ...
                                        <div class="row ">
                                            <div class="form-group col-md-8">
                                                <label for="baslik">* <?=$diller['adminpanel-form-text-1138']?></label>
                                            ...
                                            <div class="form-group col-md-12 ">
                                                    <label  for="inputGroupFile01" class="w-100">* <?=$diller['adminpanel-form-text-1139']?>  <small>( png,  jpg, jpeg, gif, svg )</small></label>
                            ...
                            <div class="row ">
                                <div class="col-md-7 ">
                                    <?php if(isset($_GET['search'])  && $_GET['search']==!null ) {?>
                                        ...
                                        <a href="pages.php?page=photo_gallery" class="btn btn-sm btn-info shadow"><?=$_GET['search']?> <i class="fa fa-times"></i></a>
                                    <?php }?>
                                </div>
                                <div class="col-md-5 text-right">
                                    <form method="GET" action="pages.php">
                                        <div class="input-group ">
                                            <input type="hidden" name="page" value="photo_gallery" >
                        ...
                        <form method="post" action="post.php?process=photo_gallery_post&status=album_multidelete">
                        <div class="w-100">
                                        ...
                                        <div class="card border">
                                            <div class="card-body">
                                               ...
                                               <div class="d-flex align-items-center justify-content-center w-100 mb-3" >
                                                   <img src="../images/gallery/<?=$row['gorsel']?>" class="img-fluid border p-1" style="width: 135px; height: 100px  " >
                                               </div>
                                                <div class="text-center mb-3">
                                                    <?=$row['baslik']?>
                                                </div>
                                                <?php
                                                    $resimler = $db->prepare("select * from galeri_resim where kat_id='$row[id]' ");
                                                    $resimler->execute();
                                                    ?>
                                                    <div class="text-center mb-3">
                                                        <a href="pages.php?page=photo_gallery_list&parent=<?=$row['id']?>" class="btn btn-pink btn-sm">
                                                            <i class="far fa-image"></i>  <?=$diller['adminpanel-form-text-1141']?> (<?=$resimler->rowCount()?>)
                                                        </a>
                                                    </div>
                                                ...
                                                <div class="text-center bg-light pt-2 pb-2">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=photo_gallery&status_update=<?=$row['id']?><?=$sayfa?><?=$searchPage?>">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=photo_gallery&status_update=<?=$row['id']?><?=$sayfa?><?=$searchPage?>">
                                                            <div class="d-flex align-items-center">
                                                    ...
                                                    <?php }?>
                                                    <a class="btn btn-sm btn-warning" target="_blank" href="<?=$ayar['site_url']?>galeri/<?=$row['seo_url']?>/" data-toggle="tooltip" data-placement="top" title="<?=$diller['adminpanel-form-text-1132']?>">
                            ...
                            <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light border-top  ">
                                ...
                                <nav aria-label="Page navigation example " >
                                    <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                        <?php } ?>
                                        <?php if($Sayfa > 1){  ?>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=photo_gallery<?=$searchPage?>"><?=$diller['sayfalama-ilk']?></a></li>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=photo_gallery<?=$searchPage?>&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                        <?php } ?>
                                        <?php
                                        for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                            if($i == $Sayfa){
                                                ...
                                                <li class="page-item active " aria-current="page">
                                                    <a class="page-link" href="pages.php?page=photo_gallery<?=$searchPage?>&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                </li>
                                                <?php
                                            }else{
                                                ?>
                                                <li class="page-item "><a class="page-link" href="pages.php?page=photo_gallery<?=$searchPage?>&p=<?=$i?>"><?=$i?></a></li>
                                                <?php
                                            }
                                        }
                                        }
                                        ...
                                        <?php if($islemListele->rowCount() <=0) { } else { ?>
                                            <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=photo_gallery<?=$searchPage?>&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=photo_gallery<?=$searchPage?>&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                            <?php }} ?>
                                        <?php if($Sayfa >= 1){?>
                                    </ul>
                                </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=photo_gallery_edit',
                type: 'post',
                data: {postID: postID},
