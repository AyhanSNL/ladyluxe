...
if ($yetki['demo'] != '1') {
    ...
    foreach ($position as $idler) {
        ...
        try {
            ...
            $query = $db->query("UPDATE blog_kat SET sira = '$count' WHERE id = '$idler2'");
...
if ($yetki['demo'] != '1') {
    if ($_GET['status_update'] == !null) {
        ...
        $statusCek = $db->prepare("select * from blog_kat where id=:id ");
        ...
        if ($statusCek->rowCount() > '0') {
            ...
            $guncelle = $db->prepare("UPDATE blog_kat SET
                 durum=:durum
         ...
         ");
            ...
            if ($sonuc) {
                header('Location:' . $ayar['panel_url'] . 'pages.php?page=blogs_categories' . $searchPage . ''.$sayfa.'');
            } else {
                echo 'Veritabanı Hatası';
            }

        } else {
            header('Location:' . $ayar['panel_url'] . 'pages.php?page=blogs_categories');
        }
...
?>
<title><?=$diller['adminpanel-menu-text-54-2']?> - <?=$panelayar['baslik']?></title>
<style>
    .ssa:before{
        display: none;
    }
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
                    ...
                    <div class="row align-items-center" >
                        <div class="col-md-8" >
                            <div class="page-title-nav">
                                ...
                                <a href="pages.php?page=blogs_categories"><i class="fa fa-angle-right"></i> <?=$diller['adminpanel-menu-text-54-2']?></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php if($yetki['icerik_yonetim'] == '1' && $yetki['blog_hizmet'] == '1' ) {
            ...
            $Say = $db->query("select * from blog_kat where $search dil='$_SESSION[dil]'  ");
            $ToplamVeri = $Say->rowCount();
            $Limit = 30;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from blog_kat where $search dil='$_SESSION[dil]'  order by sira ASC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <h4> <?=$diller['adminpanel-menu-text-54-2']?></h4>
                            </div>
                            <div class="new-buttonu-main">
                                <a  class="btn btn-success text-white "  data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-1095']?></a>
                                <div class="dropdown">
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse " id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3">
                                    <form action="post.php?process=blog_post&status=blog_cat_add" method="post" >
                                        <div class="row ">
                                            <div class="form-group col-md-12 text-center bg-white text-dark border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-1095']?></h5>
                                            </div>
                                        </div>
                            ...
                            <div class="row ">
                                <div class="col-md-7 ">
                                    <?php if(isset($_GET['search'])  && $_GET['search']==!null ) {?>
                                        ...
                                        <a href="pages.php?page=blogs_categories" class="btn btn-sm btn-info shadow"><?=$_GET['search']?> <i class="fa fa-times"></i></a>
                                    <?php }?>
                                </div>
                                <div class="col-md-5 text-right">
                                    <form method="GET" action="pages.php">
                                        <div class="input-group ">
                                            <input type="hidden" name="page" value="blogs_categories" >
                        ...
                        <div class="w-100">
                            ...
                            <div class="w-100  mb-2 ">
                                <form method="post" action="post.php?process=blog_post&status=cat_multidelete">
                                        ...
                                        <?php foreach ($islemCek as $row) {
                                            ...
                                            <tr id="<?php echo $row['id'] ?>" style="cursor: move">
                                                ...
                                                <td width="128" style="min-width: 128px">
                                                    <a href="<?=$ayar['site_url']?>bloglar/kategori//<?=$row['seo_url']?>/" target="_blank" class="btn btn-sm btn-outline-dark rounded">
                                                        <i class="fa fa-external-link-alt"></i>  <?=$diller['adminpanel-form-text-1049']?>
                                                    </a>
                                                </td>
                                                <td width="85">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=blogs_categories&status_update=<?=$row['id']?><?=$searchPage?><?=$sayfa?>">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=blogs_categories&status_update=<?=$row['id']?><?=$searchPage?><?=$sayfa?>">
                                                            <div class="d-flex align-items-center">
                                                ...
                                                <td class="text-right" style="min-width: 100px">
                                                    ...
                                                    <a href="" data-href="post.php?process=blog_post&status=cat_delete&no=<?=$row['id']?>"  data-toggle="modal" data-target="#confirm-delete"  class="btn btn-sm btn-danger "><i class="fa fa-trash" ></i></a>
                                                </td>
                                            </tr>
                                        <?php }?>
                                        </tbody>
                                    ...
                                    <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light  ">
                                        ...
                                        <nav aria-label="Page navigation example " >
                                            <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                                <?php } ?>
                                                <?php if($Sayfa > 1){  ?>
                                                    <li class="page-item "><a class="page-link " href="pages.php?page=blogs_categories<?=$searchPage?>"><?=$diller['sayfalama-ilk']?></a></li>
                                                    <li class="page-item "><a class="page-link " href="pages.php?page=blogs_categories<?=$searchPage?>&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                                <?php } ?>
                                                <?php
                                                for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                                    if($i == $Sayfa){
                                                        ...
                                                        <li class="page-item active " aria-current="page">
                                                            <a class="page-link" href="pages.php?page=blogs_categories<?=$searchPage?>&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                        </li>
                                                        <?php
                                                    }else{
                                                        ?>
                                                        <li class="page-item "><a class="page-link" href="pages.php?page=blogs_categories<?=$searchPage?>&p=<?=$i?>"><?=$i?></a></li>
                                                        <?php
                                                    }
                                                }
                                                }
                                                ...
                                                <?php if($islemListele->rowCount() <=0) { } else { ?>
                                                    <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                        <li class="page-item"><a class="page-link" href="pages.php?page=blogs_categories<?=$searchPage?>&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                        <li class="page-item"><a class="page-link" href="pages.php?page=blogs_categories<?=$searchPage?>&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                                    <?php }} ?>
                                                <?php if($Sayfa >= 1){?>
                                            </ul>
                                        </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=blog_cat_edit',
                type: 'post',
                data: {postID: postID},
