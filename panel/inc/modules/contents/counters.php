    ...
    if ($yetki['demo'] != '1') {
        ...
        foreach ($position as $idler) {
            ...
            try {
                ...
                $query = $db->query("UPDATE sayac SET sira = '$count' WHERE id = '$idler2'");
...
if(isset($_GET['status_update'])  ) {
    if($yetki['demo'] != '1' ) {
        if($_GET['status_update'] == !null  ) {
            ...
            $statusCek = $db->prepare("select * from sayac where id=:id ");
...
?>
<title><?=$diller['adminpanel-menu-text-64']?> - <?=$panelayar['baslik']?></title>
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
                    ...
                    <div class="row align-items-center" >
                        <div class="col-md-8" >
                            <div class="page-title-nav">
                                ...
                                <a href="pages.php?page=counters"><i class="fa fa-angle-right"></i> <?=$diller['adminpanel-menu-text-64']?></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php if($yetki['icerik_yonetim'] == '1' && $yetki['icerik_diger'] == '1' ) {
            ...
            $Say = $db->query("select * from sayac where dil='$_SESSION[dil]'");
            $ToplamVeri = $Say->rowCount();
            $Limit = 30;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from sayac where dil='$_SESSION[dil]'  order by sira ASC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <h4> <?=$diller['adminpanel-menu-text-64']?></h4>
                                <?=$diller['adminpanel-form-text-1124']?> <?=$ToplamVeri?>
                            </div>
                            <div class="new-buttonu-main">
                                <a  class="btn btn-info text-white  " href="pages.php?page=theme_client_counter"><?=$diller['adminpanel-form-text-838']?></a>
                                <a  class="btn btn-success text-white "   data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-1173']?></a>
                            </div>
                        </div>
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse " id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3 mt-3">
                                    <form action="post.php?process=counters_post&status=add" method="post" >
                                        <div class="row">
                                            <div class="form-group col-md-12 text-center bg-white text-dark  border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-1173']?></h5>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <input type="number" autocomplete="off" min="1"  name="count" id="count"  required class="form-control">
                                            </div>
                                            <div class="form-group col-md-6">
                                            ...
                                            <div class="form-group col-md-6 ">
                                                <label  for="plus" class="w-100" ><?=$diller['adminpanel-form-text-1177']?></label>
                                                <div class="custom-control custom-switch custom-switch-lg">
                                                    <input type="hidden" name="plus" value="0"">
                                                    <input type="checkbox" class="custom-control-input" id="plus" name="plus" value="1"   >
                                                    <label class="custom-control-label" for="plus"></label>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="col-md-4" id="<?php echo $row['id'] ?>" style="cursor: move">
                                        <div class="card border">
                                            <div class="card-body">
                                                ...
                                                <?php }else { ?>
                                                    <div class="w-100 d-flex align-items-center justify-content-center" style="font-size: 40px ; height: 40px">
                                                        <i class="fas fa-ban" style="color: #ccc"></i>
                                                    </div>
                                                <?php }?>
                                                ...
                                                <div class="text-center mb-3 mt-3" >
                                                    <div class="btn btn-outline-dark btn-sm" style="font-size: 25px ;">
                                                        <?=$row['count']?>
                                                        <?php if($row['plus'] == '1' ) {?>
                                                        +
                                                        <?php }?>
                                                ...
                                                <div class="text-center bg-light pt-2 pb-2">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=counters&status_update=<?=$row['id']?><?=$sayfa?>">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=counters&status_update=<?=$row['id']?><?=$sayfa?>">
                                                            <div class="d-flex align-items-center">
                                                    ...
                                                    <a href="" data-href="post.php?process=counters_post&status=delete&no=<?=$row['id']?>"  data-toggle="modal" data-target="#confirm-delete"  class="btn btn-sm btn-danger "><i class="fa fa-trash" ></i></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light border-top  ">
                                ...
                                <nav aria-label="Page navigation example " >
                                    <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                        <?php } ?>
                                        <?php if($Sayfa > 1){  ?>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=counters"><?=$diller['sayfalama-ilk']?></a></li>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=counters&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                        <?php } ?>
                                        <?php
                                        for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                            if($i == $Sayfa){
                                                ...
                                                <li class="page-item active " aria-current="page">
                                                    <a class="page-link" href="pages.php?page=counters&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                </li>
                                                <?php
                                            }else{
                                                ?>
                                                <li class="page-item "><a class="page-link" href="pages.php?page=counters&p=<?=$i?>"><?=$i?></a></li>
                                                <?php
                                            }
                                        }
                                        }
                                        ...
                                        <?php if($islemListele->rowCount() <=0) { } else { ?>
                                            <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=counters&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=counters&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                            <?php }} ?>
                                        <?php if($Sayfa >= 1){?>
                                    </ul>
                                </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=counters_edit',
                type: 'post',
                data: {postID: postID},
