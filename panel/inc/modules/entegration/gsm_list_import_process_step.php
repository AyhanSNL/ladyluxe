...
$Sorgula = $db->prepare("select * from gsm_import where id=:id ");
$Sorgula->execute(array(
    'id' => $id
));
$row = $Sorgula->fetch(PDO::FETCH_ASSOC);
$xml_file = simplexml_load_file("inc/input/gsm/$row[dosya]");
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            <div class="page-title-nav">
                                ...
                                <a href="pages.php?page=gsm_list_import_process&id=<?=$_GET['id']?>"><i
        ...
        <?php if ($yetki['entegrasyon'] == '1' && $yetki['entegrasyon_sms'] == '1') { ?>
            ...
            if(!$xml_file) { ?>
                <div class="card p-xl-5">
                    ...
                    <div class="mt-3">
                        <a href="pages.php?page=gsm_list_import_process&id=<?=$_GET['id']?>"
                           class="btn btn-primary"><?= $diller['adminpanel-text-138'] ?></a>
                    </div>
                </div>
            <?php }else{?>
                <?php if($_POST && isset($_POST['sync'])  ) {
                    if($_POST['ana_etiket'] && $_POST['no_etiket']) {
                        $anaTag = $_POST['ana_etiket'];
                        $gsmTag = $_POST['no_etiket'];
                        $isimTag = $_POST['isim_etiket'];
                        if(isset($xml_file->$anaTag)) {
                            if(isset($xml_file->$anaTag->$gsmTag)  )
                            {
                                ?>
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="card p-3">
                                            <div>
                                                <a href="pages.php?page=gsm_list_import_process&id=<?=$_GET['id']?>" class="btn btn-outline-dark  mb-2 btn-sm  " >
                                            ...
                                            <form action="post.php?process=gsm_laststep_post&status=import" method="post">
                                                ...
                                                <input type="hidden" name="gsmtag" value="<?=$gsmTag?>">
                                                <input type="hidden" name="isimtag" value="<?=$isimTag?>">
                            ...
                            <?php }else{
                                ...
                                <div class="col-md-12">
                                    <div class="card p-3">
                                        <div>
                                            <a href="pages.php?page=gsm_list_import_process&id='.$_GET['id'].'" class="btn btn-dark  mb-2 btn-sm  " >
                                                <i class="fa fa-arrow-left"></i> '.$diller['adminpanel-text-138'].'
                                            </a>
                                        </div>
                        ...
                        header('Location:'.$ayar['panel_url'].'pages.php?page=gsm_list_import_process&id='.$_GET['id'].'');
                        exit();
                    }
                }else {
                    header('Location:'.$ayar['site_url'].'404');
