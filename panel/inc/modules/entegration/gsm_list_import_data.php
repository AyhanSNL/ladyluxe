...
$dosyaSql = $db->prepare("select * from gsm_import where id=:id ");
...
$Say = $db->query("select * from sms_numaralar where xml_id='$ID' ");
$ToplamVeri = $Say->rowCount();
$Limit = 50;
$Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
$Goster = $Sayfa * $Limit - $Limit;
$GorunenSayfa = 5;
$islemListele = $db->query("select * from sms_numaralar where xml_id='$ID' order by id DESC limit $Goster,$Limit");
        ...
        <?php if ($yetki['entegrasyon'] == '1' && $yetki['entegrasyon_eposta'] == '1') { ?>
            <?php if($row['durum'] == '0' ) {?>
                <div class="card p-xl-5">
                    ...
                    <div class="mt-3">
                        <a href="<?= $ayar['panel_url'] ?>pages.php?page=gsm_list_import"
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card p-3 mb-3">
                            <div>
                                <a href="pages.php?page=gsm_list_import" class="btn btn-outline-dark  mb-2 btn-sm  " >
                        ...
                        <div class="w-100 alert-warning border-warning border p-3 text-dark mb-3">
                          <i class="fa fa-info-circle"></i> 
                            <?=$diller['adminpanel-form-text-2085']?>
                        </div>
                                    ...
                                    <?php foreach ($islemCek as $row) {?>
                                        <tr>
                                            <td style="font-weight: 500; min-width: 150px">
                                                <?php if($row['isim'] == !null ) {?>
                                                    <?=$row['isim']?>
                                                <?php }else { ?>
                                                        <?=$diller['adminpanel-form-text-2084']?>
                                                <?php }?>
                                            </td>
                                            <td >
                                ...
                                <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light  ">
                                    ...
                                    <nav aria-label="Page navigation example " >
                                        <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                            <?php } ?>
                                            <?php if($Sayfa > 1){  ?>
                                                <li class="page-item "><a class="page-link " href="pages.php?page=gsm_list_import_data&id=<?=$ID?>"><?=$diller['sayfalama-ilk']?></a></li>
                                                <li class="page-item "><a class="page-link " href="pages.php?page=gsm_list_import_data&id=<?=$ID?>&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                            <?php } ?>
                                            <?php
                                            for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                                if($i == $Sayfa){
                                                    ...
                                                    <li class="page-item active " aria-current="page">
                                                        <a class="page-link" href="pages.php?page=gsm_list_import_data&id=<?=$ID?>&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                    </li>
                                                    <?php
                                                }else{
                                                    ?>
                                                    <li class="page-item "><a class="page-link" href="pages.php?page=gsm_list_import_data&id=<?=$ID?>&p=<?=$i?>"><?=$i?></a></li>
                                                    <?php
                                                }
                                            }
                                            }
                                            ...
                                            <?php if($islemListele->rowCount() <=0) { } else { ?>
                                                <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                    <li class="page-item"><a class="page-link" href="pages.php?page=gsm_list_import_data&id=<?=$ID?>&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                    <li class="page-item"><a class="page-link" href="pages.php?page=gsm_list_import_data&id=<?=$ID?>&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                                <?php }} ?>
                                            <?php if($Sayfa >= 1){?>
                                        </ul>
                                    </nav>
