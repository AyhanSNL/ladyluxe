...
$grupSqsl = $db->prepare("select * from urun_varyant where id=:id ");
...
if(isset($_GET['search'])  ) {
    if(strip_tags(htmlspecialchars($_GET['search'])) <= '0'  ) {
        header('Location:'.$ayar['panel_url'].'pages.php?page=product_variants_list&parent='.$parentGet.'');
        exit();
    }
}
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
        ...
        <?php if($yetki['katalog'] == '1' && $yetki['varyant'] == '1') {
            ...
            $Say = $db->query("select * from urun_varyant_ozellik where var_id='$parentGet' $search");
            $ToplamVeri = $Say->rowCount();
            $Limit = 50;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from urun_varyant_ozellik where var_id='$parentGet' $search order by id DESC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <a href="pages.php?page=product_variants" class="btn btn-outline-dark btn-sm mb-2 d-inline-block"><i class="fas fa-arrow-left"></i> <?=$diller['adminpanel-text-164']?></a>
                                <h4> <?=$diller['adminpanel-menu-text-7']?> <i class="fa fa-caret-right"></i> <?=$grupRow['baslik']?></h4>
                                <div>
                            ...
                            <div class="new-buttonu-main">
                                <div class="new-buttonu-main">
                                    <a  class="btn btn-success text-white "  data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-1886']?></a>
                                </div>
                            </div>
                        </div>
                        <div id="accordion" class="accordion">
                            <div class="collapse " id="genelAcc" data-parent="#accordion">
                                <form action="post.php?process=catalog_post2&status=variant_add" method="post" class="border mb-2 " >
                                    <input type="hidden" name="var_id" value="<?=$grupRow['id']?>">
                                    <div class="text-center bg-white text-dark border-bottom ">
                                        <h5 style="margin-top: 0; padding-top: 10px;"> <?=$diller['adminpanel-form-text-1886']?></h5>
                                    </div>
                                    <div class="tab-content bg-white rounded-bottom">
                                        <div class="tab-pane active p-4" id="one" role="tabpanel" >
                                            <div class="row d-flex align-items-center justify-content-center">
                                                <div class="form-group col-md-7">
                                                    <label for="baslik">* <?=$diller['adminpanel-form-text-1885']?></label>
                            ...
                            <div class="row ">
                                <div class="col-md-7 ">
                                    <?php if(isset($_GET['search'])  && $_GET['search']==!null ) {?>
                                        ...
                                        <a href="pages.php?page=product_variants_list&parent=<?=$parentGet?>" class="btn btn-sm btn-info shadow"><?=trim(strip_tags($_GET['search']))?> <i class="fa fa-times"></i></a>
                                    <?php }?>
                                </div>
                                <div class="col-md-5 text-right">
                                    <form method="GET" action="pages.php">
                                        <div class="input-group ">
                                            <input type="hidden" name="page" value="product_variants_list" id="" required class="form-control">
                        ...
                        <div class="w-100">
                            ...
                            <div class="w-100  mb-2 ">
                                <form method="post" action="post.php?process=catalog_post2&status=variant_multidelete">
                                    <input type="hidden" name="var_id" value="<?=$grupRow['id']?>">
                                    ...
                                    <table class="table table-hover mb-0 table-bordered  ">
                                        ...
                                        <tr>
                                            ...
                                            <th><?=$diller['adminpanel-form-text-1885']?></th>
                                    ...
                                    <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light  ">
                                        ...
                                        <nav aria-label="Page navigation example " >
                                            <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                                <?php } ?>
                                                <?php if($Sayfa > 1){  ?>
                                                    <li class="page-item "><a class="page-link " href="pages.php?page=product_variants_list&parent=<?=$parentGet?><?=$searchPage?>"><?=$diller['sayfalama-ilk']?></a></li>
                                                    <li class="page-item "><a class="page-link " href="pages.php?page=product_variants_list&parent=<?=$parentGet?><?=$searchPage?>&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                                <?php } ?>
                                                <?php
                                                for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                                    if($i == $Sayfa){
                                                        ...
                                                        <li class="page-item active " aria-current="page">
                                                            <a class="page-link" href="pages.php?page=product_variants_list&parent=<?=$parentGet?><?=$searchPage?>&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                        </li>
                                                        <?php
                                                    }else{
                                                        ?>
                                                        <li class="page-item "><a class="page-link" href="pages.php?page=product_variants_list&parent=<?=$parentGet?><?=$searchPage?>&p=<?=$i?>"><?=$i?></a></li>
                                                        <?php
                                                    }
                                                }
                                                }
                                                ...
                                                <?php if($islemListele <=0) { } else { ?>
                                                    <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                        <li class="page-item"><a class="page-link" href="pages.php?page=product_variants_list&parent=<?=$parentGet?><?=$searchPage?>&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                        <li class="page-item"><a class="page-link" href="pages.php?page=product_variants_list&parent=<?=$parentGet?><?=$searchPage?>&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                                    <?php }} ?>
                                                <?php if($Sayfa >= 1){?>
                                            </ul>
                                        </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=variant_list_edit',
                type: 'post',
                data: {postID: postID},
