...
if($_POST) {
    if ($yetki['demo'] != '1') {
        ...
        foreach ($position as $idler) {
            ...
            try {
                $query = $db->query("UPDATE urun_ozellik_grup SET sira = '$count' WHERE id = '$idler2'");
                $guncelle = $db->prepare("UPDATE filtre_ozellik_grup SET
                       sira=:sira 
                 WHERE kontrol={$idler2}      
                ");
...
if(isset($_GET['status_update'])  ) {
    if ($yetki['demo'] != '1') {
        if ($_GET['status_update'] == !null) {
            ...
            $statusCek = $db->prepare("select * from urun_ozellik_grup where id=:id ");
         ...
         ");
                ...
                if ($sonuc) {
                    header('Location:' . $ayar['panel_url'] . 'pages.php?page=product_features' . $searchPage . ''.$pageGet.'');
                } else {
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
        ...
        <?php if($yetki['katalog'] == '1' && $yetki['varyant'] == '1') {
            ...
            $Say = $db->query("select * from urun_ozellik_grup $search");
            $ToplamVeri = $Say->rowCount();
            $Limit = 50;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from urun_ozellik_grup $search order by sira ASC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <h4> <?=$diller['adminpanel-menu-text-6']?></h4>
                                <div>
                            ...
                            <div class="new-buttonu-main">
                                <div class="new-buttonu-main">
                                    <a  class="btn btn-success text-white "  data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-1857']?></a>
                                </div>
                            </div>
                        </div>
                        <div id="accordion" class="accordion">
                            <div class="collapse" id="genelAcc" data-parent="#accordion">
                                <form action="post.php?process=catalog_post2&status=feature_group_add" method="post" class="border mb-5 " >
                                    <div class="text-center bg-white text-dark border-bottom ">
                                        <h5 style="margin-top: 0; padding-top: 10px;"> <?=$diller['adminpanel-form-text-1857']?></h5>
                                    </div>
                                        ...
                                        <div class="tab-pane active p-4" id="one" role="tabpanel" >
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-7">
                                                    <label for="baslik">* <?=$diller['adminpanel-form-text-1858']?></label>
                            ...
                            <div class="row ">
                                <div class="col-md-7 ">
                                    <?php if(isset($_GET['search'])  && $_GET['search']==!null ) {?>
                                        ...
                                        <a href="pages.php?page=product_features" class="btn btn-sm btn-info shadow"><?=trim(strip_tags($_GET['search']))?> <i class="fa fa-times"></i></a>
                                    <?php }?>
                                </div>
                                <div class="col-md-5 text-right">
                                    <form method="GET" action="pages.php">
                                        <div class="input-group ">
                                            <input type="hidden" name="page" value="product_features" id="" required class="form-control">
                        ...
                        <div class="w-100">
                            ...
                            <div class="w-100  mb-2 ">
                                <form method="post" action="post.php?process=catalog_post2&status=feature_group_multidelete">
                                <div class="table-responsive ">
                                    <table class="table table-hover mb-0 table-bordered  ">
                                        ...
                                        <tr>
                                            ...
                                            <th><?=$diller['adminpanel-form-text-1858']?></th>
                                            <th></th>
                                        ...
                                        <?php foreach ($islemListele as $row) { 
                                            $ozellikListesiCount = $db->prepare("select id from urun_ozellik where grup_id=:grup_id ");
                                            ...
                                            <tr id="<?php echo $row['id'] ?>" style="cursor: move" >
                                                ...
                                                <td width="145" class="text-center">
                                                    <a class="btn btn-sm btn-primary " href="pages.php?page=product_features_list&parent=<?=$row['id']?>">
                                                        <div class="d-flex align-items-center">
                                                            <?=$diller['adminpanel-form-text-1859']?> (<?=$ozellikListesiCount->rowCount()?>)
                                                        </div>
                                                    </a>
                                                </td>
                                                <td width="85">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=product_features&status_update=<?=$row['id']?><?=$searchPage?><?=$pageGet?>">
                                                            <div class="d-flex align-items-center">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=product_features&status_update=<?=$row['id']?><?=$searchPage?><?=$pageGet?>">
                                                            <div class="d-flex align-items-center">
                                    ...
                                    <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light  ">
                                        ...
                                        <nav aria-label="Page navigation example " >
                                            <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                                <?php } ?>
                                                <?php if($Sayfa > 1){  ?>
                                                    <li class="page-item "><a class="page-link " href="pages.php?page=product_features<?=$searchPage?>"><?=$diller['sayfalama-ilk']?></a></li>
                                                    <li class="page-item "><a class="page-link " href="pages.php?page=product_features<?=$searchPage?>&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                                <?php } ?>
                                                <?php
                                                for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                                    if($i == $Sayfa){
                                                        ...
                                                        <li class="page-item active " aria-current="page">
                                                            <a class="page-link" href="pages.php?page=product_features<?=$searchPage?>&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                        </li>
                                                        <?php
                                                    }else{
                                                        ?>
                                                        <li class="page-item "><a class="page-link" href="pages.php?page=product_features<?=$searchPage?>&p=<?=$i?>"><?=$i?></a></li>
                                                        <?php
                                                    }
                                                }
                                                }
                                                ...
                                                <?php if($islemListele <=0) { } else { ?>
                                                    <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                        <li class="page-item"><a class="page-link" href="pages.php?page=product_features<?=$searchPage?>&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                        <li class="page-item"><a class="page-link" href="pages.php?page=product_features<?=$searchPage?>&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                                    <?php }} ?>
                                                <?php if($Sayfa >= 1){?>
                                            </ul>
                                        </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=feature_group_edit',
                type: 'post',
                data: {postID: postID},
