    ...
    if ($yetki['demo'] != '1') {
        ...
        foreach ($position as $idler) {
            ...
            try {
                ...
                $query = $db->query("UPDATE story_grup SET sira = '$count' WHERE id = '$idler2'");
...
if ($yetki['demo'] != '1') {
    if ($_GET['status_update'] == !null) {
        ...
        $statusCek = $db->prepare("select * from story_grup where id=:id ");
        ...
        if ($statusCek->rowCount() > '0') {
         ...
         ");
            ...
            if ($sonuc) {
                header('Location:' . $ayar['panel_url'] . 'pages.php?page=stories' . $sayfa . '');
            } else {
...
?>
<title><?=$diller['adminpanel-menu-text-71']?> - <?=$panelayar['baslik']?></title>
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
        ...
        <?php if($yetki['modul'] == '1' &&  $yetki['modul_diger'] == '1') {
            ...
            $Say = $db->query("select * from story_grup where dil='$_SESSION[dil]'");
            $ToplamVeri = $Say->rowCount();
            $Limit = 25;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from story_grup where dil='$_SESSION[dil]'  order by sira ASC limit $Goster,$Limit");
                    ...
                    <div class="card p-3">
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <h4> <?=$diller['adminpanel-menu-text-71']?></h4>
                            </div>
                            <div class="new-buttonu-main">
                                <a  class="btn btn-info text-white " href="post.php?process=stories_post&status=theme_settings"><?=$diller['adminpanel-form-text-838']?></a>
                                <a  class="btn btn-success text-white "   data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-919']?></a>
                            </div>
                        </div>
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse" id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3">
                                    <form action="post.php?process=stories_post&status=add_group" method="post" enctype="multipart/form-data" >
                                        <div class="row ">
                                            <div class="form-group col-md-12 text-center bg-white text-dark  border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-919']?></h5>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row ">
                                            <div class="form-group col-md-8">
                                                ...
                                                <input type="text" autocomplete="off"  name="baslik" id="baslik" <?php if($stAyar['tur'] == '1' ) { ?>required<?php } ?> class="form-control">
                                            </div>
                                            ...
                                            <div class="form-group col-md-12 ">
                                                    <label  for="inputGroupFile01" class="w-100">* <?=$diller['adminpanel-form-text-922']?>  <small>( png,  jpg, jpeg, webp, jp2, gif )</small></label>
                        ...
                        <form method="post" action="post.php?process=stories_post&status=multidelete">
                        <div class="w-100">
                                        ...
                                        <div class="card border">
                                            <div class="card-body">
                                               ...
                                               <div class="d-flex align-items-center justify-content-center w-100 mb-3" >
                                                   <img src="../i/stories/<?=$row['gorsel']?>" class="img-fluid border p-1" style="width: 90px; height: 90px; border-radius: 100px;" >
                                               </div>
                                                <div class="text-center mb-3">
                                                    <?=$row['baslik']?>
                                                </div>
                                                <?php if($stAyar['tur'] == '1' ) {
                                                    $storiler = $db->prepare("select * from story where grup_id='$row[grup_id]' ");
                                                    $storiler->execute();
                                                    ?>
                                                    <div class="text-center mb-3">
                                                        <a href="pages.php?page=story_item_list&grup_id=<?=$row['grup_id']?>" class="btn btn-warning btn-block">
                                                            <?=$diller['adminpanel-form-text-923']?> (<?=$storiler->rowCount()?>)
                                                        </a>
                                                    </div>
                                                <?php }?>
                                                <div class="text-center bg-light pt-2 pb-2">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=stories&status_update=<?=$row['id']?><?=$sayfa?>">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=stories&status_update=<?=$row['id']?><?=$sayfa?>">
                                                            <div class="d-flex align-items-center">
                                                    ...
                                                    <a href="" data-href="post.php?process=stories_post&status=delete&no=<?=$row['id']?>"  data-toggle="modal" data-target="#confirm-delete"  class="btn btn-sm btn-danger "><i class="fa fa-trash" ></i></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light border-top  ">
                                ...
                                <nav aria-label="Page navigation example " >
                                    <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                        <?php } ?>
                                        <?php if($Sayfa > 1){  ?>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=stories"><?=$diller['sayfalama-ilk']?></a></li>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=stories&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                        <?php } ?>
                                        <?php
                                        for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                            if($i == $Sayfa){
                                                ...
                                                <li class="page-item active " aria-current="page">
                                                    <a class="page-link" href="pages.php?page=stories&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                </li>
                                                <?php
                                            }else{
                                                ?>
                                                <li class="page-item "><a class="page-link" href="pages.php?page=stories&p=<?=$i?>"><?=$i?></a></li>
                                                <?php
                                            }
                                        }
                                        }
                                        ...
                                        <?php if($islemListele->rowCount() <=0) { } else { ?>
                                            <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=stories&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=stories&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                            <?php }} ?>
                                        <?php if($Sayfa >= 1){?>
                                    </ul>
                                </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=stories_edit',
                type: 'post',
                data: {postID: postID},
