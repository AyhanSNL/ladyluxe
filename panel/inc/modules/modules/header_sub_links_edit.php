...
<?php if($_POST['postID']  ) {
    ...
    $queryControl = $db->prepare("select * from header_menu where id='$_POST[postID]' ");
            ...
            <div class="modal-content" style="box-shadow: 0 0 10px 0 rgba(0,0,0,.3); border:1px solid #ccc">
                ...
                <div class="modal-body  pt-1 pl-4 pr-4 pb-4">
                    <div class="w-100  border-bottom  pt-2 pb-2 mb-3">
                        <h4><?=$diller['adminpanel-form-text-883']?></h4>
                    </div>
                    <form action="post.php?process=header_links_post&status=submenu_edit" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="menu_id" value="<?=$row['id']?>" >
                        <input type="hidden" name="parent_id" value="<?=$row['ust_id']?>" >
                        <div class="row ">
                            ...
                            <div id="manuel-choise-edit" class="col-md-12 " <?php if($row['url_tur'] != '2' ) { ?>style="display: none;"<?php }?>   >
                                <div class="w-100 p-3 border bg-light up-arrow-2 ">
                                        <input type="text" name="manuel_url" value="<?=$row['url']?>" autocomplete="off" placeholder="https://"  class="form-control">
                                </div>
                            </div>
                        </div>
