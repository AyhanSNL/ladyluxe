...
$ustMenuCek = $db->prepare("select baslik,id,ust_id,dil from header_mobil where id='$_GET[parent]' ");
$ustMenuCek->execute();
...
if($detay['ust_id'] > '0' ) {
    $fatherMenu = $db->prepare("select baslik,id,ust_id from header_mobil where id='$detay[ust_id]' ");
    $fatherMenu->execute();
    $dad = $fatherMenu->fetch(PDO::FETCH_ASSOC);
}

if($ustMenuCek->rowCount()<='0' && $_GET['parent']==!null && isset($_GET['parent'])  ) {
    header('Location:'.$ayar['panel_url'].'pages.php?page=mobile_header_links');
    die();
}
    ...
    if ($_GET['status_update'] == !null) {
        ...
        if ($statusCek->rowCount() > '0') {
         ...
         ");
            ...
            if ($sonuc) {
                header('Location:' . $ayar['panel_url'] . 'pages.php?page=mobile_header_list&parent=' . $_GET['parent'] . '' . $sayfa . '' . $searchPage . '');
            } else {
...
?>
<title><?=$detay['baslik']?> <?=$diller['adminpanel-form-text-882']?> - <?=$panelayar['baslik']?></title>
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
                    ...
                    <div class="row align-items-center" >
                        <div class="col-md-8" >
                            <div class="page-title-nav">
                                ...
                                <?php if($detay['ust_id'] == '0' ) {?>
                                    <a href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>"><i class="fa fa-angle-right"></i>  <?=$detay['baslik']?></a>
                                <?php }else { ?>
                                    <a href="pages.php?page=mobile_header_list&parent=<?=$dad['id']?>"><i class="fa fa-angle-right"></i>  <?=$dad['baslik']?></a>
                                    <a href="javascript:Void(0)"><i class="fa fa-angle-right"></i> <?=$detay['baslik']?></a>
                                <?php }?>
                                <a href="javascript:Void(0)"><i class="fa fa-angle-right"></i> <?=$diller['adminpanel-form-text-882']?></a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <?php if($yetki['yapilandirma'] == '1') {
            ...
            $Say = $db->query("select * from header_mobil where $search ust_id='$_GET[parent]'");
            $ToplamVeri = $Say->rowCount();
            $Limit = 1;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from header_mobil where $search ust_id='$_GET[parent]'  order by sira ASC limit $Goster,$Limit");
                ...
                <div class="<?php if($panelayar['panel_nav'] == '1'   ) { ?>col-md-9<?php }else{?>col-md-12<?php } ?>">
                    <div class="card p-3">
                        <div class="w-100 d-flex align-items-center justify-content-between flex-wrap pb-2 mb-2">
                            <div class="w-100">
                                <?php if($detay['ust_id'] == '0' ) {?>
                                    <a href="pages.php?page=mobile_header_links" class="btn btn-outline-dark btn-sm mb-2 d-inline-block"><i class="fas fa-arrow-left"></i> <?=$diller['adminpanel-text-164']?></a>
                                <?php }else { ?>
                                    <a href="pages.php?page=mobile_header_list&parent=<?=$detay['ust_id']?>" class="btn btn-outline-dark btn-sm mb-2 d-inline-block"><i class="fas fa-arrow-left"></i> <?=$diller['adminpanel-text-164']?></a>
                                <?php }?>
                            </div>
                            <?php if($detay['ust_id'] == '0' ) {?>
                                <div class="new-buttonu-main-top">
                                    <div class="new-buttonu-main-left">
                                        <h4><?=$detay['baslik']?> <i class="fa fa-caret-right"></i> <?=$diller['adminpanel-form-text-882']?> (<?=$ToplamVeri?>)</h4>
                                    </div>
                                    <div class="new-buttonu-main">
                                        <a  class="btn btn-success text-white "  data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-881']?></a>
                                    </div>
                                </div>
                            <?php }else {?>
                                <div class="new-buttonu-main-top">
                                    <div class="new-buttonu-main-left">
                                        <div style="font-size: 18px ;"><strong><?=$dad['baslik']?> <i class="fa fa-caret-right"></i>  <?=$detay['baslik']?> </strong> <i class="fa fa-caret-right"></i> <?=$diller['adminpanel-form-text-882']?> (<?=$ToplamVeri?>)</div>
                                    </div>
                                    <div class="new-buttonu-main">
                                        <a  class="btn btn-success text-white "  data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-881']?></a>
                                    </div>
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse" id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3">
                                    <form action="post.php?process=mobile_header_post&status=submenu_add" method="post" >
                                        ...
                                        <div class="row ">
                                            <div class="form-group col-md-12 text-center bg-white text-dark border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-881']?></h5>
                                            </div>
                                        </div>
                            ...
                            <div class="row ">
                                <div class="col-md-7 ">
                                    <?php if(isset($_GET['search'])  && $_GET['search']==!null ) {?>
                                        ...
                                        <a href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>" class="btn btn-sm btn-info shadow"><?=$_GET['search']?> <i class="fa fa-times"></i></a>
                                    <?php }?>
                                </div>
                                <div class="col-md-5 text-right">
                                    <form method="GET" action="pages.php">
                                        <div class="input-group ">
                                            <input type="hidden" name="page" value="mobile_header_list" >
                        ...
                        <div class="w-100">
                            ...
                            <form method="post" action="post.php?process=mobile_header_post&status=submenu_multidelete&parent=<?=$_GET['parent']?>">
                                            ...
                                            <tr id="<?php echo $row['id'] ?>" style="cursor: move">
                                                ...
                                                <?php if($detay['ust_id'] == '0') {?>
                                                    <td style="min-width: 165px" width="165">
                                                        <a href="pages.php?page=mobile_header_list&parent=<?=$row['id']?>" class="btn btn-sm alert-warning border border-warning text-dark  rounded">
                                                ...
                                                <td width="85">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&status_update=<?=$row['id']?><?=$sayfa?><?=$searchPage?>">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&status_update=<?=$row['id']?><?=$sayfa?><?=$searchPage?>">
                                                            <div class="d-flex align-items-center">
                                                ...
                                                <td class="text-right" style="min-width: 100px">
                                                    <a href="javascript:Void(0)" data-id="<?=$row['id']?>" class="btn btn-sm btn-primary duzenleAjax" data-toggle="tooltip" data-placement="top" title="<?=$diller['adminpanel-text-159']?>"><i class="fa fa-eye" ></i></a>
                                                        <a href="" data-href="post.php?process=mobile_header_post&status=submenu_delete&no=<?=$row['id']?>&parent=<?=$_GET['parent']?>"  data-toggle="modal" data-target="#confirm-delete"  class="btn btn-sm btn-danger "><i class="fa fa-trash" ></i></a>
                                                </td>
                                            </tr>
                                        <?php }?>
                                        </tbody>
                                ...
                                <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light  ">
                                    ...
                                    <nav aria-label="Page navigation example " >
                                        <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                            <?php } ?>
                                            <?php if($Sayfa > 1){  ?>
                                                <li class="page-item "><a class="page-link " href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?><?=$searchPage?>"><?=$diller['sayfalama-ilk']?></a></li>
                                                <li class="page-item "><a class="page-link " href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&p=<?=$Sayfa - 1?><?=$searchPage?>"><?=$diller['sayfalama-onceki']?></a></li>
                                            <?php } ?>
                                            <?php
                                            for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                                if($i == $Sayfa){
                                                    ...
                                                    <li class="page-item active " aria-current="page">
                                                        <a class="page-link" href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&p=<?=$i?><?=$searchPage?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                    </li>
                                                    <?php
                                                }else{
                                                    ?>
                                                    <li class="page-item "><a class="page-link" href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&p=<?=$i?><?=$searchPage?>"><?=$i?></a></li>
                                                    <?php
                                                }
                                            }
                                            }
                                            ...
                                            <?php if($islemListele->rowCount() <=0) { } else { ?>
                                                <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                    <li class="page-item"><a class="page-link" href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&p=<?=$Sayfa + 1?><?=$searchPage?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                    <li class="page-item"><a class="page-link" href="pages.php?page=mobile_header_list&parent=<?=$_GET['parent']?>&p=<?=$Sayfa_Sayisi?><?=$searchPage?>"><?=$diller['sayfalama-son']?></a></li>
                                                <?php }} ?>
                                            <?php if($Sayfa >= 1){?>
                                        </ul>
                                    </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=mobile_header_list_edit',
                type: 'post',
                data: {postID: postID},
