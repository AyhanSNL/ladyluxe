...
$storuGrp = $db->prepare("select * from story_grup where grup_id=:grup_id ");
$storuGrp->execute(array(
        'grup_id' => $_GET['grup_id']
));
$grupRow = $storuGrp->fetch(PDO::FETCH_ASSOC);


if($storuGrp->rowCount()<='0'  ) {
    ...
    if ($yetki['demo'] != '1') {
        ...
        foreach ($position as $idler) {
            ...
            try {
                ...
                $query = $db->query("UPDATE story SET sira = '$count' WHERE id = '$idler2'");
...
if(isset($_GET['status_update'])  ) {
    if ($yetki['demo'] != '1') {
        if ($_GET['status_update'] == !null) {
            ...
            $statusCek = $db->prepare("select * from story where id=:id ");
         ...
         ");
                ...
                if ($sonuc) {
                    header('Location:' . $ayar['panel_url'] . 'pages.php?page=story_item_list&grup_id=' . $_GET['grup_id'] . '' . $sayfa . '');
                } else {
...
?>
<title><?=$grupRow['baslik']?> - <?=$diller['adminpanel-menu-text-71']?> - <?=$panelayar['baslik']?></title>
...
<div class="wrapper" style="margin-top: 0;">
    <div class="container-fluid">
                    ...
                    <div class="row align-items-center" >
                        <div class="col-md-8" >
                            <div class="page-title-nav">
                                ...
                                <a href="pages.php?page=stories"><i class="fa fa-angle-right"></i> <?=$diller['adminpanel-menu-text-71']?></a>
        ...
        <?php if($yetki['modul'] == '1' &&  $yetki['modul_diger'] == '1') {
            ...
            $Say = $db->query("select * from story where grup_id='$_GET[grup_id]' ");
            $ToplamVeri = $Say->rowCount();
            $Limit = 9;
            $Sayfa_Sayisi = ceil($ToplamVeri/$Limit); if($Sayfa > $Sayfa_Sayisi){$Sayfa = 1;}
            $Goster = $Sayfa * $Limit - $Limit;
            $GorunenSayfa = 5;
            $islemListele = $db->query("select * from story where grup_id='$_GET[grup_id]'  order by sira ASC limit $Goster,$Limit");
            ...
            <div class="row">
                ...
                <div class="<?php if($panelayar['panel_nav'] == '1'   ) { ?>col-md-9<?php }else{?>col-md-12<?php } ?>">
                    <div class="card p-3">
                        <div class="w-100">
                            <a href="pages.php?page=stories" class="btn btn-outline-dark btn-sm mb-2 d-inline-block"><i class="fas fa-arrow-left"></i> <?=$diller['adminpanel-text-164']?></a>
                        </div>
                        <div class="new-buttonu-main-top">
                            <div class="new-buttonu-main-left">
                                <h5> <?=$diller['adminpanel-menu-text-71']?> <i class="fa fa-caret-right"></i> <?=$grupRow['baslik']?></h5>
                            </div>
                            <div class="new-buttonu-main">
                                <a  class="btn btn-success text-white "   data-toggle="collapse" data-target="#genelAcc" aria-expanded="false" aria-controls="collapseForm"><i class="fas fa-plus-circle "></i> <?=$diller['adminpanel-form-text-931']?></a>
                            </div>
                        </div>
                        ...
                        <div id="accordion" class="accordion">
                            <div class="collapse " id="genelAcc" data-parent="#accordion">
                                <div class="w-100 border pl-3 pr-3 mb-3">
                                    <form action="post.php?process=stories_post&status=story_add" method="post" enctype="multipart/form-data" >
                                        ...
                                        <div class="row ">
                                            <div class="form-group col-md-12 text-center bg-white text-dark  border-bottom mb-0 ">
                                                <h5> <?=$diller['adminpanel-form-text-931']?></h5>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row ">
                                            <div class="form-group col-md-6">
                                                <label for="baslik">* <?=$diller['adminpanel-form-text-924']?></label>
                                            ...
                                            <div class="form-group col-md-3">
                                                <label for="saniye">* <?=$diller['adminpanel-form-text-926']?> </label>
                                                <input type="number" autocomplete="off"  min="1" name="saniye" id="saniye" required class="form-control">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label for="url_adres"><?=$diller['adminpanel-form-text-921']?> (<?=$diller['adminpanel-form-text-933']?>)</label>
                                                <input type="text" name="url_adres" id="url_adres"  placeholder="https://"   autocomplete="off" class="form-control">
                                            </div>
                                            <div class="form-group col-md-12">
                                                <label for="url_baslik"><?=$diller['adminpanel-form-text-934']?> (<?=$diller['adminpanel-form-text-935']?>)</label>
                                                <input type="text" autocomplete="off"  name="url_baslik" id="url_baslik"  class="form-control">
                                            </div>
                                            <div class="form-group col-md-12 ">
                                                    <label  for="inputGroupFile01" class="w-100">* <?=$diller['adminpanel-form-text-925']?>  <small>( png,  jpg, jpeg )</small></label>
                                        ...
                                        <div class="card border">
                                            <div class="card-body">
                                               <div class="d-flex align-items-center justify-content-center w-100 mb-3" >
                                                   <img src="../i/stories/<?=$row['gorsel']?>" class="img-fluid border" style="height: 140px" >
                                               </div>
                                                ...
                                                <div class="text-center mb-3">
                                                    <div class="btn btn-warning btn-sm">
                                                        <?=$row['saniye']?> <?=$diller['adminpanel-form-text-936']?>
                                                    </div>
                                                </div>
                                                <div class="text-center bg-light pt-2 pb-2">
                                                    <?php if($row['durum'] == '0' ) {?>
                                                        <a class="btn btn-sm btn-outline-danger " href="pages.php?page=story_item_list&status_update=<?=$row['id']?>&grup_id=<?=$_GET['grup_id']?><?=$sayfa?>">
                                                    ...
                                                    <?php }?>
                                                    <?php if($row['durum'] == '1' ) {?>
                                                        <a class="btn btn-sm btn-success " href="pages.php?page=story_item_list&status_update=<?=$row['id']?>&grup_id=<?=$_GET['grup_id']?><?=$sayfa?>">
                                                            <div class="d-flex align-items-center">
                                                    ...
                                                    <a href="" data-href="post.php?process=stories_post&status=story_delete&no=<?=$row['id']?>&grup_id=<?=$_GET['grup_id']?>"  data-toggle="modal" data-target="#confirm-delete"  class="btn btn-sm btn-danger "><i class="fa fa-trash" ></i></a>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div id="SayfalamaElementi" class="w-100 p-2  border-bottom bg-light border-top  ">
                                ...
                                <nav aria-label="Page navigation example " >
                                    <ul class="pagination pagination-sm justify-content-end mb-0 ">
                                        <?php } ?>
                                        <?php if($Sayfa > 1){  ?>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=story_item_list&grup_id=<?=$_GET['grup_id']?>"><?=$diller['sayfalama-ilk']?></a></li>
                                            <li class="page-item "><a class="page-link " href="pages.php?page=story_item_list&grup_id=<?=$_GET['grup_id']?>&p=<?=$Sayfa - 1?>"><?=$diller['sayfalama-onceki']?></a></li>
                                        <?php } ?>
                                        <?php
                                        for($i = $Sayfa - $GorunenSayfa; $i < $Sayfa + $GorunenSayfa +1; $i++){ if($i > 0 and $i <= $Sayfa_Sayisi){
                                            if($i == $Sayfa){
                                                ...
                                                <li class="page-item active " aria-current="page">
                                                    <a class="page-link" href="pages.php?page=story_item_list&grup_id=<?=$_GET['grup_id']?>&p=<?=$i?>"><?=$i?><span class="sr-only">(current)</span></a>
                                                </li>
                                                <?php
                                            }else{
                                                ?>
                                                <li class="page-item "><a class="page-link" href="pages.php?page=story_item_list&grup_id=<?=$_GET['grup_id']?>&p=<?=$i?>"><?=$i?></a></li>
                                                <?php
                                            }
                                        }
                                        }
                                        ...
                                        <?php if($islemListele->rowCount() <=0) { } else { ?>
                                            <?php if($Sayfa != $Sayfa_Sayisi){?>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=story_item_list&grup_id=<?=$_GET['grup_id']?>&p=<?=$Sayfa + 1?>"><?=$diller['sayfalama-sonraki']?></a></li>
                                                <li class="page-item"><a class="page-link" href="pages.php?page=story_item_list&grup_id=<?=$_GET['grup_id']?>&p=<?=$Sayfa_Sayisi?>"><?=$diller['sayfalama-son']?></a></li>
                                            <?php }} ?>
                                        <?php if($Sayfa >= 1){?>
                                    </ul>
                                </nav>
...
<script type='text/javascript'>
    $(document).ready(function(){
        ...
        $('.duzenleAjax').click(function(){
            ...
            // AJAX request
            $.ajax({
                url: 'masterpiece.php?page=stories_item_edit',
                type: 'post',
                data: {postID: postID},
