...
if($odemeayar['pos_tur']  == 'iyzico' ) {
    $siparisVarmi = $db->prepare("select * from siparisler where siparis_no=:siparis_no and onay=:onay ");
    $siparisVarmi->execute(array(
        'siparis_no' => htmlspecialchars($_GET['sID']),
        'onay' => '0'
    ));
...
<?php if($odemeayar['pos_tur'] == 'iyzico' ) {
    require_once('includes/template/vpos/iyzico/IyzipayBootstrap.php');
